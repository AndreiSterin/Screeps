var destinationPool =  {
    ...
    cleanup_dead_creeps: function(type) {
        ...
        var names_to_delete = [];
        for (var name in Memory.creep_destination) {
    ...
    create_sources_map_from_scratch: function(type) {
        ...
        for (id in Memory.creep_destination) {
            var destination = Memory.creep_destination[id];
    ...
    init_destinations: function(type) {
        ...
        Memory.creep_destination = {};
    },
    get: function(type, name) {
        if (Memory.creep_destination[name]) {
            var id = Memory.creep_destination[name];
        ...
        for (var dest_idx in destinations) {
            ...
            if (Memory.destination[id] < 1) {
                ...
                Memory.creep_destination[name] = id;
                return Game.getObjectById(id);
            }
        }
    },
    free: function(type, name) {
        if (Memory.creep_destination[name]) {
            var id = Memory.creep_destination[name];
            delete Memory.creep_destination[name];
            Memory.destination[id] = Memory.destination[id] - 1;
        }
    }
};
